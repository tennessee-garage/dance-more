...
Only the gamma half is here so far; `FrameEncoder` (frame -> SEND_DATA
payloads, global brightness, white balance) is #63 and lands in this module
too. The primitives below exist first because `pixels.py` needs them for
anything that averages or mixes colours - `PixelFrame.to_tiles()` and
`blend()` both work in linear light - and the preview and the floor must
agree about what a byte means, so there is exactly one gamma curve, defined
here and imported everywhere else.
