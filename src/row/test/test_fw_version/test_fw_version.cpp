...
// row_fw_version() pulls ROW_FW_VERSION and the build-injected FW_GIT_SHA/
// FW_DIRTY (tools/fw_version_env.py) together; native/test_native builds
// still see real values since the script runs there too.
void test_row_fw_version_reports_build_identity() {
    FirmwareVersion v = row_fw_version();
    TEST_ASSERT_EQUAL_UINT16(ROW_FW_VERSION, v.version);
...
int main(int, char **) {
    ...
    RUN_TEST(test_row_fw_version_reports_build_identity);
